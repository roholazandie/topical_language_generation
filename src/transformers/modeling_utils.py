...
class PreTrainedModel(nn.Module):
    ...
    def fusion(self, logits, scores, config):
        ...
        if config.fusion_method == "method1":
            ...
            index_first_occur = list(np.cumsum(np.sort(scores)[::-1]) > CUM_PROB_THRESHOLD).index(True)
            index_candidate_tokens = np.argsort(scores)[::-1][:index_first_occur + 1]
            # scores[index_candidate_tokens]
            ...
            id2 = token_probs < 0*scores
            #total_probs[id1] = 1.0*token_probs[id1] + 0.0 * scores[id1]
            #total_probs[id2] = 0.0*token_probs[id2] + 1.0 * scores[id2]
    ...
    def _generate_topical2(
        ...
        while cur_len < max_length:
            ...
            log_probs = np.log(token_probs.numpy())
            log_probs = log_probs[log_probs > -1e30]
            plt.hist(log_probs, bins=100, color="r")
            plt.show()

            log_topic_probs = np.log(topic_probs)
            log_topic_probs = log_topic_probs[log_topic_probs>-12]
            plt.hist(log_topic_probs, bins=100, color='y')
            plt.show()


            total_probs = self.fusion(next_token_logits.squeeze(0), topic_probs, method="method4")
